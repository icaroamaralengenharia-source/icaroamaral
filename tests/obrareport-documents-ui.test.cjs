...
test("renderiza cards technical_report, rdo e vistoria com metadados", () => {
  const { target } = render([
    { sourceType: "technical_report", sourceId: "rep-1", title: "Relatorio A", clientId: "client-a", projectId: "project-a", status: "draft", createdBy: "Ana", createdAt: "2026-08-28T10:00:00.000Z", documentAvailable: true, pdfAvailable: false, fileUrl: "/api/obrareport/documents/doc-report/file", canContinue: true },
...
test("mostra apenas acoes aplicaveis por documento", () => {
  const { target, calls } = render([
    { sourceType: "technical_report", sourceId: "rep-1", title: "Relatorio A", canContinue: true, documentAvailable: true, pdfAvailable: false, fileUrl: "/api/obrareport/documents/doc-report/file" },
  ...
  assert.deepEqual(buttons(target.children[0]).map((button) => button.textContent), ["Abrir", "Baixar arquivo", "Continuar"]);
