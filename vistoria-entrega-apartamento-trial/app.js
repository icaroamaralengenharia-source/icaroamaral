(function () {
  ...
  const nodes = {
    ...
    startInspection: document.querySelector("[data-start-inspection]"),
    finalizeInspectionButton: document.querySelector("[data-finalize-inspection]")
  };

  function createInspectionId() { return window.crypto && typeof window.crypto.randomUUID === "function" ? window.crypto.randomUUID() : "10000000-1000-4000-8000-" + String(Date.now()).slice(-12).padStart(12, "0"); }

  function trialAccess() { return window.ApartmentHandoverAccess && window.ApartmentHandoverAccess.getState ? window.ApartmentHandoverAccess.getState() : null; }

  function canCreateTrialInspection() { const access = trialAccess(); return !access || access.can_create !== false || state.inspection.status === "completed"; }
  ...
  function showView(name) { if (name !== "identification" && !canCreateTrialInspection()) name = "dashboard"; state.currentView = name; nodes.views.forEach((view) => { view.hidden = view.dataset.view !== name; }); render(); }
  ...
  function renderTrialControls() {
    const access = trialAccess();
    const exhausted = access && access.status === "trial_exhausted" && access.can_create === false && state.inspection.status !== "completed";
    if (nodes.startInspection) {
      nodes.startInspection.disabled = Boolean(exhausted);
      nodes.startInspection.textContent = exhausted ? "Teste concluído" : "Iniciar vistoria";
    }
    if (nodes.finalizeInspectionButton) nodes.finalizeInspectionButton.disabled = Boolean(exhausted);
  }
  ...
  function finalizeInspection() { if (!canCreateTrialInspection()) { showPdfStatus("final", "Teste concluído. Para criar novas vistorias, ative seu acesso."); return; } const c = computeSummary().counts; nodes.finalizeWarning.textContent = `${c[NOT_INSPECTED]} itens não inspecionados, ${c.NC} não conformidades e ${c.NV} itens não verificados.`; nodes.finalizeDialog.showModal(); }
  function confirmFinalize() { if (!canCreateTrialInspection()) { nodes.finalizeDialog.close(); showPdfStatus("final", "Teste concluído. Para criar novas vistorias, ative seu acesso."); return; } state.inspection.status = "completed"; state.inspection.completedAt = new Date().toISOString(); nodes.finalizeDialog.close(); showView("dashboard"); }
  ...
  async function generateInspectionPdf(mode, options = {}) {
    ...
    try {
      if (window.ApartmentHandoverAccess && !(await window.ApartmentHandoverAccess.ensureAllowed())) return;
      const pdfHeaders = window.ApartmentHandoverAccess ? window.ApartmentHandoverAccess.authenticatedHeaders({ "Content-Type": "application/json" }) : { "Content-Type": "application/json" };
      const response = await fetch(`${API_BASE_URL}/api/apartment-handover/pdf-protected`, {
        method: "POST",
        headers: pdfHeaders,
  ...
  document.querySelector("[data-start-inspection]").addEventListener("click", () => { if (!canCreateTrialInspection()) { showPdfStatus("final", "Teste concluído. Para criar novas vistorias, ative seu acesso."); return; } state.inspection.startedAt = state.inspection.startedAt || new Date().toISOString(); showView("dashboard"); });
  ...
  window.addEventListener("apartment-handover-access-changed", renderTrialControls);
