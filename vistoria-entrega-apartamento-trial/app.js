(function () {
  ...
  function ensureInspectionUuid() {
    if (isUuid(state.inspectionId)) return state.inspectionId;
    if (isUuid(state.inspection_id)) return state.inspection_id;
    const generated = window.crypto && typeof window.crypto.randomUUID === "function"
      ? window.crypto.randomUUID()
      : "10000000-1000-4000-8000-" + String(Date.now()).slice(-12).padStart(12, "0");
    state.inspectionId = generated;
    saveState();
    return generated;
  }
  ...
  async function generateInspectionPdf(mode, options = {}) {
    ...
    try {
      const headers = window.ApartmentHandoverAccess && typeof window.ApartmentHandoverAccess.authenticatedHeaders === "function"
        ? window.ApartmentHandoverAccess.authenticatedHeaders({ "Content-Type": "application/json" })
  ...
  async function buildApartmentHandoverPdfPayload(mode) {
    ...
    const inspectionId = ensureInspectionUuid();
    return {
      mode,
