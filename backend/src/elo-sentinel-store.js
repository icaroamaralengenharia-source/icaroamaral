...
const SOURCE_TABLES = {
  sentinel: { evidence: "elo_sentinel_evidences", pending_item: "elo_sentinel_pending_items", validation: "elo_sentinel_pending_items", correction: "elo_sentinel_evidences" },
  rdo: { rdo: "obrareport_rdos", rdo_document: "obrareport_generated_documents" },
  technical_report: { report: "obrareport_technical_reports", report_document: "obrareport_generated_documents" },
  generated_document: { document: "obrareport_generated_documents", document_file: "obrareport_document_files" },
  elo_budget: { budget: "elo_budget_documents", budget_version: "elo_budget_versions" },
  budget_pdf: { budget_pdf: "elo_generated_documents", document: "elo_generated_documents" }
};
function sourceTable(moduleName, entityType) { return SOURCE_TABLES[clean(moduleName, 80)] && SOURCE_TABLES[clean(moduleName, 80)][clean(entityType, 80)] || null; }
function sourceReference(event, exists = false) {
  if (!event || !event.source_module || !event.source_entity_type || !event.source_entity_id) return null;
  return { source_module: event.source_module, source_entity_type: event.source_entity_type, source_entity_id: event.source_entity_id, source_exists: Boolean(exists) };
}
...
function normalizeEvent(row, evidence = null, source = null) {
  if (!row) return null;
  const event = {
    id: row.id, institution_id: row.institution_id, company_id: row.company_id, project_id: row.project_id,
    evidence_id: row.evidence_id || null, event_type: row.event_type, source_module: row.source_module || null,
    source_entity_type: row.source_entity_type || null, source_entity_id: row.source_entity_id || null,
    title: row.title, description: row.description || null, occurred_at: row.occurred_at, created_by: row.created_by || null,
    severity: row.severity || null, status: row.status || null, metadata: row.metadata || {},
    idempotency_key: row.idempotency_key || null, evidence: evidenceSummary(evidence), created_at: row.created_at
  };
  const reference = source || sourceReference(event, false);
  if (reference) { event.source_reference = reference; event.source_exists = Boolean(reference.source_exists); }
  return event;
}
...
export function createEloSentinelStore(options = {}) {
  ...
  async function createEvent(input = {}) {
    ...
    return insert("elo_sentinel_events", Object.assign({}, input, scope, {
      ...
      event_type: clean(input.event_type || input.eventType, 120), source_module: clean(input.source_module || input.sourceModule, 80) || null,
      source_entity_type: clean(input.source_entity_type || input.sourceEntityType, 80) || null, source_entity_id: clean(input.source_entity_id || input.sourceEntityId, 160) || null,
      title: clean(input.title, 240), description: clean(input.description, 4000) || null,
      occurred_at: clean(input.occurred_at || input.occurredAt, 80) || now(), created_by: clean(input.created_by || input.createdBy, 140) || null,
      severity: clean(input.severity, 40) || null, status: clean(input.status, 40) || null,
      metadata: input.metadata && typeof input.metadata === "object" ? input.metadata : {},
      idempotency_key: clean(input.idempotency_key || input.idempotencyKey, 220) || null
    }), normalizeEvent);
  }
  async function findTimelineEventByIdempotencyKey(input = {}) { const scope = assertScope(input); const key = clean(input.idempotency_key || input.idempotencyKey, 220); if (!key) return null; const result = await database().from("elo_sentinel_events").select("*").eq("institution_id", scope.institution_id).eq("company_id", scope.company_id).eq("project_id", scope.project_id).eq("idempotency_key", key).maybeSingle(); rethrowStoreError(result.error); return result.data ? clone(normalizeEvent(result.data)) : null; }
  async function createOperationalTimelineEvent(input = {}) { const existing = await findTimelineEventByIdempotencyKey(input); if (existing) return { event: existing, idempotent: true }; return { event: await createEvent(input), idempotent: false }; }
  async function findSourceReference(event = {}, scopeInput = {}) { const scope = assertScope(scopeInput); if (!event.source_module || !event.source_entity_type || !event.source_entity_id) return sourceReference(event, false); const table = sourceTable(event.source_module, event.source_entity_type); if (!table) return sourceReference(event, false); try { let query = database().from(table).select("id").eq("id", clean(event.source_entity_id, 160)); if (table.startsWith("elo_sentinel_") || table.startsWith("obrareport_") || table.startsWith("elo_budget_")) query = query.eq("institution_id", scope.institution_id); if (event.source_module !== "elo_budget" && event.source_module !== "budget_pdf" && table !== "obrareport_generated_documents" && table !== "obrareport_document_files") query = query.eq("project_id", scope.project_id); const result = await query.maybeSingle(); return sourceReference(event, !result.error && Boolean(result.data)); } catch (_) { return sourceReference(event, false); } }
  ...
  async function listTimelineByProject(input = {}) { return listOperationalTimelineByProject(input); }
  async function listOperationalTimelineByProject(input = {}) { const scope = assertScope(input); const limit = Number(input.limit) || 50; const offset = Number(input.offset) || 0; let query = database().from("elo_sentinel_events").select("*").eq("institution_id", scope.institution_id).eq("company_id", scope.company_id).eq("project_id", scope.project_id); ["event_type","source_module","source_entity_type","severity","status","created_by"].forEach((field) => { if (input[field]) query = query.eq(field, input[field]); }); if (input.source_entity_id) query = query.eq("source_entity_id", input.source_entity_id); query = applyCommonFilters(query, input, "occurred_at"); const search = clean(input.search, 200).toLowerCase(); query = query.order("occurred_at", { ascending: false }).order("created_at", { ascending: false }).range(offset, offset + limit); const result = await query; rethrowStoreError(result.error); let rows = result.data || []; if (search) rows = rows.filter((item) => [item.title, item.description, item.source_module, item.source_entity_type, JSON.stringify(item.metadata || {})].some((value) => String(value || "").toLowerCase().includes(search))); const items = await Promise.all(rows.map(async (item) => { const event = normalizeEvent(item); const evidence = event.evidence_id ? await findEvidenceById(event.evidence_id, scope) : null; const source = await findSourceReference(event, scope).catch(() => sourceReference(event, false)); return clone(normalizeEvent(item, evidence, source)); })); return pageResult(items, { limit, offset }); }
  ...
  return { createEvidence, listEvidences: (scope) => listEvidencesByProject(scope).then((result) => result.items), createEvent, listTimeline: (scope) => listTimelineByProject(scope).then((result) => result.items), createEvidenceWithEvent, findEvidenceById, findEvidenceByIdempotencyKey, listEvidencesByProject, listTimelineByProject, createOperationalTimelineEvent, findTimelineEventByIdempotencyKey, listOperationalTimelineByProject, findSourceReference, createPendingItem, findPendingItemById, findPendingItemByIdempotencyKey, listPendingItemsByProject, updatePendingItem, linkEvidenceToPendingItem, listPendingItemEvidences, listPendingItemEvents };
}

export function createEloSentinelMemoryStore() {
  const evidences = []; const events = []; const pendingItems = []; const pendingLinks = []; const inputSources = Array.isArray(arguments[0] && arguments[0].sources) ? arguments[0].sources : [];
  function nextId(prefix, list) { return prefix + "-" + String(list.length + 1).padStart(4, "0"); }
  function matchesFilters(item, filters = {}) { if (filters.evidence_type && item.evidence_type !== filters.evidence_type) return false; if (filters.source && item.source !== filters.source) return false; if (filters.event_type && item.event_type !== filters.event_type) return false; if (filters.source_module && item.source_module !== filters.source_module) return false; if (filters.source_entity_type && item.source_entity_type !== filters.source_entity_type) return false; if (filters.source_entity_id && item.source_entity_id !== filters.source_entity_id) return false; if (filters.status && item.status !== filters.status) return false; if (filters.priority && item.priority !== filters.priority) return false; if (filters.severity && item.severity !== filters.severity) return false; if (filters.created_by && item.created_by !== filters.created_by) return false; if (filters.responsible_user_id && item.responsible_user_id !== filters.responsible_user_id) return false; if (filters.date_from && String(item.occurred_at || item.created_at) < filters.date_from) return false; if (filters.date_to && String(item.occurred_at || item.created_at) > filters.date_to) return false; if (filters.due_from && String(item.due_at || "") < filters.due_from) return false; if (filters.due_to && String(item.due_at || "9999") > filters.due_to) return false; if (filters.search) { const q = clean(filters.search, 200).toLowerCase(); if (![item.title, item.description, item.source_module, item.source_entity_type, JSON.stringify(item.metadata || {})].some((v) => String(v || "").toLowerCase().includes(q))) return false; } return true; }
  ...
  async function createEvent(input = {}) { const scope = assertScope(input); const row = normalizeEvent(Object.assign({}, input, scope, { id: input.id || nextId("event", events), evidence_id: input.evidence_id || input.evidenceId || null, event_type: input.event_type || input.eventType, source_module: input.source_module || input.sourceModule || null, source_entity_type: input.source_entity_type || input.sourceEntityType || null, source_entity_id: input.source_entity_id || input.sourceEntityId || null, severity: input.severity || null, status: input.status || null, idempotency_key: input.idempotency_key || input.idempotencyKey || null, occurred_at: input.occurred_at || input.occurredAt || now(), created_by: input.created_by || input.createdBy || null, metadata: input.metadata || {}, created_at: input.created_at || now() })); events.push(row); return clone(row); }
  async function findTimelineEventByIdempotencyKey(input = {}) { const scope = assertScope(input); const key = clean(input.idempotency_key || input.idempotencyKey, 220); if (!key) return null; const item = events.find((event) => scoped(event, scope) && event.idempotency_key === key); return item ? clone(item) : null; }
  async function createOperationalTimelineEvent(input = {}) { const existing = await findTimelineEventByIdempotencyKey(input); if (existing) return { event: existing, idempotent: true }; return { event: await createEvent(input), idempotent: false }; }
  async function findSourceReference(event = {}, scopeInput = {}) { const scope = assertScope(scopeInput); if (!event.source_module || !event.source_entity_type || !event.source_entity_id) return sourceReference(event, false); const id = event.source_entity_id; let exists = false; if (event.source_module === "sentinel" && event.source_entity_type === "evidence") exists = evidences.some((item) => item.id === id && scoped(item, scope)); else if (event.source_module === "sentinel" && ["pending_item","validation"].includes(event.source_entity_type)) exists = pendingItems.some((item) => item.id === id && scoped(item, scope)); else exists = Array.isArray(inputSources) && inputSources.some((item) => item.id === id && item.source_module === event.source_module && item.source_entity_type === event.source_entity_type && scoped(Object.assign({}, scope, item), scope)); return sourceReference(event, exists); }
  async function createEvidenceWithEvent(input = {}, eventInput = {}) { const scope = assertScope(input); const existing = await findEvidenceByIdempotencyKey(input); if (existing) return { evidence: existing, event: clone(eventForEvidence(existing.id, scope) || null), idempotent: true }; const evidence = await createEvidence(input); try { const event = await createEvent(Object.assign({}, eventInput, scope, { evidence_id: evidence.id, source_entity_id: eventInput.source_entity_id || eventInput.sourceEntityId || evidence.id })); return { evidence, event, idempotent: false }; } catch (error) { const index = evidences.findIndex((item) => item.id === evidence.id && scoped(item, scope)); if (index >= 0) evidences.splice(index, 1); throw error; } }
  ...
  async function listTimelineByProject(input = {}) { return listOperationalTimelineByProject(input); }
  async function listOperationalTimelineByProject(input = {}) { const scope = assertScope(input); const limit = Number(input.limit) || 50; const offset = Number(input.offset) || 0; const rows = events.filter((item) => scoped(item, scope) && matchesFilters(item, input)).sort((a, b) => String(b.occurred_at).localeCompare(String(a.occurred_at)) || String(b.created_at).localeCompare(String(a.created_at))); const items = await Promise.all(rows.map(async (event) => normalizeEvent(event, evidences.find((evidence) => evidence.id === event.evidence_id && scoped(evidence, scope)) || null, await findSourceReference(event, scope)))); return pageResult(items.slice(offset, offset + limit + 1).map((item) => clone(item)), { limit, offset }); }
